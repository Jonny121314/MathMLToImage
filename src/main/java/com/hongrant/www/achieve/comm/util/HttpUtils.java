...
public class HttpUtils {
    ...
    private static final int MAX_TIMEOUT = 600000;// 18秒
     ...
     */
    public static String doPostByToken(String url, Map<String, Object> tokenMap) throws IOException, InterruptedException {
    	String tokenName ="";
    	String tokenValue ="";
    	if (tokenMap != null) {
        	tokenName = MapUtils.getString(tokenMap, "tokenName");
        	tokenValue = MapUtils.getString(tokenMap, "tokenValue");
            }
    ...
    try {
        HttpPost httpPost = new HttpPost(url);
        httpPost.setHeader("Cookie", tokenName+"="+tokenValue);
        response = httpclient.execute(httpPost);
        Integer codeInteger = response.getStatusLine().getStatusCode();
        while (codeInteger!=200) {
        	response.close();
        	Thread.sleep(2*60*1000);//休息2分钟
            response = httpclient.execute(httpPost);
            codeInteger = response.getStatusLine().getStatusCode();
		}
        entity = response.getEntity();
        if (entity != null) {
            InputStream instream = entity.getContent();
